...
*
*	FELIPE ANDRE DEZ/2018 versao BUGADO (BUSCA, INSERIR)
********************************************************/
...
{
	TLista aux, novo;	
	if( buscar((*L), numero) == NULL) //se nao encontrar
	{
		...
		if (novo)
		{
			...
			{		
				aux = *L;		
				while (aux->prox) //no inicio aux = *L, limitando -se ao ultimo antes de NULL
				{
					if((aux->valor < numero) && (aux->prox->valor > numero)) //insere na ordem crescente
					{
						novo->valor = numero;
						novo->prox = aux->prox;
						aux->prox = novo;			 
...
//Busca primeira ocorrencia, retorna endereco do 'no'
{
	...
	while ((aux))// && (aux->prox) && (aux->prox->valor <= numero))  //while (aux != NULL)
	{
		if (aux->valor == numero)
		{
			return aux;
